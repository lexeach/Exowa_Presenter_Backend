...
function parseDateAndTime(speechText = "") {
  ...
  /* -------- TIME LOGIC -------- */
  if (text.includes("2") || text.includes("दो") || text.includes("dopahar 2")) {
    demoTime = "2:00 PM";
  } else if (text.includes("6") || text.includes("छह") || text.includes("shaam 6")) {
    demoTime = "6:00 PM";
  } else if (text.includes("11") || text.includes("ग्यारह")) {
    demoTime = "11:00 AM";
  }
...
<Response>
  <Speak language="hi-IN" voice="WOMAN">
    नमस्ते। मैं Exowa से बोल रही हूँ। क्या आप अपने बच्चे के लिए demo schedule करना चाहेंगे? कृपया समय बोलकर बताएं।
  </Speak>
  <Gather action="${processUrl}" method="POST" inputType="speech" timeout="8" language="hi-IN">
    <Speak language="hi-IN">
      जैसे आप कह सकते हैं: कल शाम 6 बजे या कल सुबह 11 बजे
    </Speak>
  </Gather>
  <Speak language="hi-IN">
    हमें आपकी आवाज़ प्राप्त नहीं हुई। कृपया बाद में पुनः प्रयास करें।
  </Speak>
</Response>`;
...
router.post("/process-slot", async (req, res) => {
  try {
    ...
    const speechText = req.body.Speech || req.body.speech || "";
    ...
    if (!speechText.trim()) {
      ...
      return res.send(`
<Response>
  <Speak language="hi-IN">कृपया समय स्पष्ट रूप से बताएं।</Speak>
</Response>`);
    }
...
<Response>
  <Speak language="hi-IN">
    धन्यवाद। आपका demo ${demoDate.toDateString()} को ${demoTime} पर confirm हो गया है।
  </Speak>
</Response>`;
